...
namespace Nova
{
    ...
    public class CheckpointSerializer : IDisposable
    {
        ...
        public void Open()
        {
            using (var stream = File.Open(path, FileMode.OpenOrCreate, System.IO.FileAccess.ReadWrite))
            {
                file = new FileStream(stream.SafeFileHandle, System.IO.FileAccess.ReadWrite);
            }
